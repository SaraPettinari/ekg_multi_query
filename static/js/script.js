...
function generate_dagre(data) {
  ...
  // Function to retrieve unique values associated with the key 'robot'
  const uniqueRobots = new Set();
  data.nodes.forEach(item => uniqueRobots.add(item.e.robot));
  var robots = Array.from(uniqueRobots);

  // Creating a dictionary from unique values mapped to pastel colors
  const colorDict = {};
  robots.forEach((value, index) => {
  ...
  for (e in data.edges) {
    ...
    var edge_type = edge.edge_properties.CorrelationType
